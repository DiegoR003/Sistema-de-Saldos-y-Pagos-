...
// Configuración de errores para respuesta JSON
...
try {
    ...
    // Preparar variables visuales (Igual que en cotizacion_pdf.php)
    ...
    // 2. OBTENER ITEMS Y MAPEAR TEXTOS (La función clave para que se vea bien)
    $stI = $pdo->prepare("SELECT grupo, opcion, valor FROM cotizacion_items WHERE cotizacion_id = ? ORDER BY id ASC");
    $stI->execute([$id]);
    $itemsDb = $stI->fetchAll(PDO::FETCH_ASSOC);

    // Función copiada de cotizacion_pdf.php para mantener consistencia
    function getInfoServicio($grupo, $opcion) {
        ...
        $map = [
            ...
            'reposteo' => ['t' => 'Community Manager', 'd' => 'Es la persona encargada de responder todos los mensajes y comentarios en un horario de 9am a 5pm lunes a viernes y sábados de 9am a 2pm, excepto dias festivos.'],
            'stories' => ['t' => 'Diseño Gráfico', 'd' => 'Incluye el diseño del post y de las stories tanto de Facebook como de Instagram. También incluye adicionalmente cambios de perfil, portada e historias destacadas.'],
            'fotos' => ['t' => 'Fotografía', 'd' => '1 sesión de máximo 2 horas de producción, 30 fotografías a entregar y 1 locación. Incluye uso de drone. Si contratas una cada 2 meses estarás obligado a quedarte 4 meses en vez de 3. No incluye modelos'],
            ...
            'mkt'   => ['t' => 'Email Marketing', 'd' => 'Incluye el diseño y envío masivo de correo. No incluye base de datos'],
            'web' => ['t' => 'Sitio Web', 'd' => 'Incluye hospedaje, dominio .com, certificado SSL, correos ilimitados en cantidad. Incluye soporte en horario laboral. Debes contratar un Sistema solo si requieres bases de datos o venta en línea.']
        ];
    ...
    // 3. PREPARAR IMÁGENES BASE64 (Copiado de cotizacion_pdf.php)
    ...
    // Iconos Sociales (Placeholders Dorados)
    ...
    // 4. GENERAR HTML (Estructura idéntica al PDF de descarga)
    ob_start();
    ?>
    <!DOCTYPE html>
    <html lang="es">
    ...
    $tempFile = sys_get_temp_dir() . DIRECTORY_SEPARATOR . "Cotizacion_{$cleanName}.pdf";
    file_put_contents($tempFile, $pdfOutput);

    // 6. ENVIAR CORREO (Usando SMTP del mailer.php)
    ...
    <div style='font-family: Arial, sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 8px;'>
        ...
        <p><strong>Total:</strong> $$total MXN + IVA</p>
        <hr>
    ...
    // Asegúrate de que enviar_correo_sistema en mailer.php acepte el array de adjuntos como 5to parámetro
